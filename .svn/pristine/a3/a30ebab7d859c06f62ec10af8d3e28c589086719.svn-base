/*
	Copyright (c) 2004-2007, Visionsoft
	All Rights Reserved.
      ...
      {with(Math)return   (round(this.valueOf()*pow(10,num))/pow(10,num)).toString().search(/\./i)==-1?(round(this.valueOf()*pow(10,num))/pow(10,num)).toString()+"."+"0".fn(num):(round(this.valueOf()*pow(10,num))/pow(10,num));   
      	...
      	function GT(param1,theobjid,showtype,type){
				...
				if(istrue == 0){
					if(type == 2 ){
						if(showtype==3)
							document.all(theobjid+"span").innerHTML = "<img src=/vimgs/checkinput.gif align=absMiddle>";
						else
			...
			function SQL(param){	
				var _url= "/ServiceAction/com.velcro.base.DataAction?sql="+encode(param);
...
function addPerson(persontype){
	if(persontype!=1 && persontype !=2 && persontype !=3)
		return;
		var url = "/vhumres/base/humresbrowserm.jsp";
		var popuptitle = encode("人事卡片");
		
		var retval = window.showModalDialog("/vbase/popupmain.jsp?popuptitle="+popuptitle+"&url="+url);
		if(retval==undefined){
			return;
		}
		var a = new VBArray(retval);
   		var b = a.toArray();
		...
		if(objid != "" && objname != "") {
			...
			param.action="/ServiceAction/com.velcro.workflow.workflow.servlet.WorkflowAction?action=addoptperson";
			...
			var result=showModalDialog("/vbase/updatedialog.html", param,
