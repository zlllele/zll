...
function setformfieldsize($fields, optsize, optoutputdiv) {
	...
	$fields.each(function(i) {
		var $field = $(this);
		var maxsize = $field.attr('data-maxsize');
		if (!maxsize) {
			maxsize = $field.attr('maxlength');
		}
		$field.data('maxsize', optsize || parseInt(maxsize));// max character limit
...
jQuery(document).ready(function($) { // fire on DOM ready
	...
	var $targetfields = $("input[data-maxsize], input[maxlength], textarea[data-maxsize], textarea[maxlength]");
