...
function TableTree4J(objectName,tableTree4JDir){
	...
	TableTree4J.prototype.addGirdNode=function(dataList,id,pid,booleanOpen,order,url,target,hrefTip,hrefStatusText,classStyle,icon,iconOpen){
		var nodeText="";
		if(this.config.ischecked&&dataList)
		{
		var checboxStr="<input type=\"checkbox\" id=\"chk"+id+"\" onClick=\"checkedTreeNode('"+id+"',"+this.obj+")\" name=\""+this.config.checkBoxName+"\" value=\""+id+"\">";
		nodeText=dataList[0];
		dataList[0]=checboxStr+dataList[0];
		}
		var nod=new Node(dataList,id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,nodeText);
		this.addNode(nod);
	}
	...
	TableTree4J.prototype.codeNodeTR=function(node){//changeClassName
		...
		if(this.config.booleanHighLightRow==true&&node.booleanRoot==false){
			if(node.classStyle!=null&&node.classStyle!=""){
				if(node.nodeText=="查看某部门及下级"||node.nodeText=="查看本部门"||node.nodeText=="查看本人")
				{
					changeClassMark=" ";
		...
		if(node.classStyle!=null&&node.classStyle!=""){
			if(node.nodeText=="查看某部门及下级"||node.nodeText=="查看本部门"||node.nodeText=="查看本人")
	    	{
				str = "<tr "+changeClassMark+" id=\""+this.obj+"Trid"+node.id+"\" nodeid=\""+node.id+"\" class=\"wdback\" style=\"display:"+node.visible+"\" >";			
	    	}else{
	    		str = "<tr "+changeClassMark+" id=\""+this.obj+"Trid"+node.id+"\" nodeid=\""+node.id+"\" class=\""+node.classStyle+"\" style=\"display:"+node.visible+"\" >";			
	    	}
		}else{
			if(node.nodeText=="查看某部门及下级"||node.nodeText=="查看本部门"||node.nodeText=="查看本人")
	    	{
				str= "<tr "+changeClassMark+" id=\""+this.obj+"Trid"+node.id+"\" class=\"wdback\" nodeid=\""+node.id+"\" style=\"display:"+node.visible+"\" >";		
	    	}else{
	    		str= "<tr "+changeClassMark+" id=\""+this.obj+"Trid"+node.id+"\" nodeid=\""+node.id+"\" style=\"display:"+node.visible+"\" >";		
	    	}
		}
		return str;		
	}			
...
//Data Value Object
function Node(dataList,id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,nodeText){
...
function checkedTreeNode(nodeid,tree){
     var node = gridTree.findTreeNodeByMapId(nodeid);
	 var trElement=document.getElementById(tree.obj+"Trid"+nodeid);
	 var ischeck = document.getElementById("chk"+nodeid).checked;
	 if(ischeck){
	 	trElement.className="GridCheckedRow";
	 //changeClassName(trElement,"GridHighLightRow");	
	 //trElement.onmouseout="";
	 }else{
		 var tempNode = tree.findTreeNodeByMapId(nodeid);
		if(tempNode.nodeText=="查看某部门及下级"||tempNode.nodeText=="查看本部门"||tempNode.nodeText=="查看本人"){
			trElement.className="wdback";
		}else{
			trElement.className="";
		}
	 //changeClassName(trElement,'');
	 }
	 var childs=node.childNodes;
	 if(tree.config.checkedParentType=="1"){
     if(childs)
	 {
      var isHaveChildSelected=false;
      for(var i=0;i<childs.length;i++){
    	  if(document.getElementById("chk"+childs[i].id).checked)
		   {
		    isHaveChildSelected=true;
		    break;	
		   }
		//alert(childs[i].htmlcode);
		checkedTreeNode(childs[i].id,tree);
	   }
      if(isHaveChildSelected)
        {
  		document.getElementById("chk"+nodeid).checked=true;
  	    }
	 }
	 }
	 //根据预设的父节点的选中逻辑值执行不同的父节点选中状态设置方法
	 if(tree.config.checkedParentType=="1"){
	   checkedParentNode(node,ischeck,tree);
	 }else{
		checkedParentNode2(node,ischeck,tree);
	 }
	}

//根据子节点的选中状态，来设置父节点的选中状态。实现逻辑功能：当子节点选中时所有上级的节点也同样选中。
function checkedParentNode(node,ischeck,tree){
	if(node.pNode.id!=tree.rootNode.id){
	var pNode = node.pNode;
 	if(ischeck){
	 document.getElementById("chk"+pNode.id).checked=ischeck;
	}else{
	     ...
	     if(!isHaveChecked)
		 {
	    //如果没有选中的子项，表示此子项的选中状态影响上级节点的选中状态。
	 	 // document.getElementById("chk"+pNode.id).checked=ischeck;
	     }		
	}
	checkedParentNode(pNode,ischeck,tree);	
	}
}

//根据子节点的选中状态，来设置父节点的选中状态2。逻辑含义：如果子节点都选中，则只需选中父节点子节点选中状态为未选
function checkedParentNode2(node,ischeck,tree){
	if(node.pNode.id!=tree.rootNode.id&&node.pNode.nodeText!="查看某部门及下级"){
		var pNode = node.pNode;
 	if(ischeck){
		  var isHaveUnChecked=false;
	     var childs=pNode.childNodes;
	     for(var i=0;i<childs.length;i++)
		 {
		   if(!document.getElementById("chk"+childs[i].id).checked)
		   {
		    isHaveUnChecked=true;
		    break;	
		   }
	     }	 
	     if(!isHaveUnChecked)
		 {
		  ...
		  document.getElementById("chk"+childs[j].id).checked=false;	
		  }
	     }else{
           	document.getElementById("chk"+pNode.id).checked=false;		
		 }	
	}else{
	document.getElementById("chk"+pNode.id).checked=ischeck;
	}
	checkedParentNode2(pNode,ischeck,tree);	
	}	
	if(node.nodeText=="查看某部门及下级"&&!document.getElementById("chk"+node.id).checked){
	//如果查看某部门及下级未选中所有下级节点也设置为为选中
		checkedByPNode(node.id,tree,false);
	}
}

//根据父节点的id和参数，设置子节点的选中状态
function checkedByPNode(id,tree,ischecked){
	var node=tree.findTreeNodeByMapId(id);
	var childs=node.childNodes;
	if(childs&&childs.length>0){
		for(var i=0;i<childs.length;i++){
			document.getElementById("chk"+childs[i].id).checked=ischecked;
			checkedByPNode(childs[i].id,tree,ischecked);
		}
	}
}

//检查子节点中是否有选中的节点，如果有返回true
var isHaveCheckedChild=false;
function ishaveCheckedChildById(id,tree){
    var node=tree.findTreeNodeByMapId(id);
	 isHaveCheckedChild=false;
	if(node)
	{
	    var childs=node.childNodes;
		if(childs&&childs.length>0){
		 ...
		 {
		   if(document.getElementById("chk"+childs[a].id).checked)
		   {
   		    isHaveCheckedChild=true;
		    break;
		   }else{
			   isHaveCheckedChild=false;  
		        }
		   if(childs[a].booleanLeaf==false){
		    ishaveCheckedChildById(childs[a].id,tree);
		   }
	     }	 
		}
	}
	return isHaveCheckedChild;
}

function initCheckedNode(checkedNodes,tree){
if(checkedNodes.length>0){
for(var i=0;i<checkedNodes.length;i++){
 document.getElementById("chk"+checkedNodes[i]).checked=true;
 var trElement=document.getElementById(tree.obj+"Trid"+checkedNodes[i]);
 trElement.className="GridCheckedRow";
}
}
}

function initCheckedNodesBygwsjqx(checkedNodes,tree){
	var wd1_checked=false;
	if(checkedNodes.length>0){
		for(var i=0;i<checkedNodes.length;i++){
		 if(checkedNodes[i]!="c_zzjg"&&checkedNodes[i]!="c_user"&&!wd1_checked)
		   {
...
function changeClassName(objectChange,classN){
	var chkobj=document.getElementById("chk"+objectChange.nodeid);
	if(chkobj&&chkobj.checked)
	{
	  objectChange.className= "GridCheckedRow";
	}else{
...
var checkedValue=[];
var allCheckBox=document.getElementsByName(tree.config.checkBoxName);
for(var i=0;i<allCheckBox.length;i++){
if(allCheckBox[i].checked)
{
 checkedValue.push(allCheckBox[i].value);
}
}
return checkedValue;
}

function selectAllChk(tree){
var allCheckBox=document.getElementsByName(tree.config.checkBoxName);
for(var i=0;i<allCheckBox.length;i++){
 allCheckBox[i].checked=true;
 allCheckBox[i].parentNode.parentNode.className="GridCheckedRow";
}  
//checkedTreeNode(tree.rootNode.id,tree);
}
function unSelectAllChk(tree){
var allCheckBox=document.getElementsByName(tree.config.checkBoxName);
for(var i=0;i<allCheckBox.length;i++){
 allCheckBox[i].checked=false;
 allCheckBox[i].parentNode.parentNode.className="";
}  
}
