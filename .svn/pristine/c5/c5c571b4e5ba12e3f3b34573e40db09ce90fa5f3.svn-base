...
function checkedTreeNode(nodeid,tree){
     ...
     if(childs) {
	     ...
	     for(var i=0;i<childs.length;i++){			
	    	 if(document.getElementById("chk"+childs[i].id).checked == true){	    		 
	    		 flag = true;
	    		 break;
	    	 }
		 }
	     if(flag) {
	      if(document.getElementById("chk"+node.id).checked==false)
	      {
	    	 if(confirm('是否取消选中的下级项！')) {
	    		 for(var i=0;i<childs.length;i++){			
	    	    	 if(document.getElementById("chk"+childs[i].id).checked == true){	
	    	    		 document.getElementById("chk"+childs[i].id).checked = false;
	    	    	 }
	    		 }
	    	 } else {
	    		 document.getElementById("chk"+node.id).checked = false;
	    		 return;
	    	 }
	    	 }
	     }
	 ...
	 if(tree.config.checkedParentType=="1") {
	     if(childs) {
		      for(var i=0;i<childs.length;i++){				
				document.getElementById("chk"+childs[i].id).checked=ischeck;
		...
		if(childs&&childs.length>0){
			for(var a=0;a<childs.length;a++){
			   ...
			   if(childs[a].booleanLeaf==false){
				   var flag = ishaveCheckedChildById(childs[a].id,tree);
				   if(flag) {
					   return true;
				   }
			   }
