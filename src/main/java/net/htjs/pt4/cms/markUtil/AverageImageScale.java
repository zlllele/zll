package net.htjs.pt4.cms.markUtil;
...
import net.htjs.pt4.cms.markUtil.ImageUtils.Position;
...
public class AverageImageScale {
	...
	public static void writeFile_ext(BufferedImage imgBuf, File destFile,String ext)
			throws IOException {
		File parent = destFile.getParentFile();
		if (!parent.exists()) {
			parent.mkdirs();
	...
	public static void imageMark(File srcFile, File destFile, int minWidth,
			...
			Color color, int size, int alpha,String ext) throws IOException {
		...
		if (width <= minWidth || height <= minHeight) {
			...
			imageMark(imgBuff, width, height, pos, offsetX, offsetY, text,
					color, size, alpha);
			writeFile_ext(imgBuff, destFile,ext);
			imgBuff = null;
		}
	}
