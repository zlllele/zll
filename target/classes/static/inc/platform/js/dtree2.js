...
function dTree(objName) {
	...
	this.icon = {
		root : '/inc/platform/theme/1/ar1.png',
		folder : '/inc/platform/theme/1/ar1.png',
		folderOpen : '/inc/platform/theme/1/ar1.png',
		node : '/inc/platform/theme/1/menu_dot.png',
...
/*
 * hzq分析的执行顺序： 1、dTree--Node 2、add--Node
 * ………………中循环执行直至数组创建完毕，开始显示“document.write(d)”显示执行toString
...
dTree.prototype.setCS = function(node) {
	...
	if (lastId == node.id)
		node._ls = true;// true表示是最后一个子节点 hzq
}
...
dTree.prototype.addNode = function(pNode) {
	...
	for (n; n < this.aNodes.length; n++) {
		// alert("pNode.id="+pNode.id+" this.aNodes[n].id="+this.aNodes[n].id+"
		// n="+n);
		if (this.aNodes[n].pid == pNode.id) {
			...
			this.setCS(cn);
			this.indent_hu(this.aNodes[n], n,
					this.aIndent[this.aIndent.length - 2]);
	...
	if (this.root.id != node.pid) {
		...
		for ( var n = 0; n < node.level; n++) {// 原来是n<Math.max((node.level-this.aNodes[1].level),0)
			...
			for ( var i = 0; i < scnode.pics.length; i++)
				if (i == n
						&& (scnode.pics[i] == 'minus'
								|| scnode.pics[i] == 'plus' || scnode.pics[i] == 'line'))
					temp = true;
		...
		if (node._hc) {// 若有子节点
			if (!this.config.useLines)
				...
				node.pics[node.pics.length] = ((node._io) ? ((node._ls && this.config.useLines) ? 'minusBottom'
						: 'minus')
						: ((node._ls && this.config.useLines) ? 'plusBottom'
								: 'plus'));
		} else
			node.pics[node.pics.length] = (this.config.useLines) ? ((node._ls) ? 'joinBottom'
					: 'join')
					: 'empty';
	}
};
...
dTree.prototype.node = function(node, nodeId, sj) {
	...
	if (node.level == -1) {
		...
		if (node._hc) {
			cssName = 'L' + (node.level + 1) + '1';
		} else {
			cssName = 'L' + (node.level + 1) + '2';
		}
	}
	var str = '<li class="' + cssName + '" id="m'+nodeId+'">' + this.indent(node, nodeId, sj);
	if (this.config.useIcons) {
		...
		// src="'+((node._io)?node.iconOpen:node.icon)+'"/>';
	}
	str += '<a id="selstr' + nodeId + '" href="javascript:Choose(\'' + node.id
			+ '\',\'' + node.name + '\',\'' + node.url + '\');disp(' + nodeId
			+ ');' + this.obj + '.o(' + nodeId
			+ ');" style="cursor:hand"><span>'
			//+ (node.level == 0||isIcon ? '<img src="' + node.icon + '" align="absMiddle">' : '')
			+ node.name + "</span></a>";
	...
	if (node._hc) {// 若有子节点
		str += '<ul id="d' + this.obj + nodeId + '" class="U'
				+ (node.level + 1) + '" style="display:'
...
dTree.prototype.o = function(id) {
	...
	if (curindex == -1)
		eDiv1 = document.getElementById('selstr1');
	else
		eDiv1 = document.getElementById('selstr' + curindex);
	eDiv1.style.backgroundColor = '';
	//eDiv1.style.color = "#000000";
	eDiv1.className=''
	if (cn._io == false) {// curren node is close add by zhouchaoyang
							// 2008-12-03
		eDiv1.className='active'
		showHelp(cn.id);
	}
...
dTree.prototype.closeLevel = function(node) {
	for ( var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].pid == node.pid && this.aNodes[n].id != node.id
				&& this.aNodes[n]._hc) {
			eDiv1 = document.getElementById('selstr' + node.id);
			eDiv1.className='';
