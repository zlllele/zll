...
strFrame+='        onclick="parent.meizzPrevM()" title="向前翻 1 月" Author=j><b Author=j>&lt;</b>';
...
strFrame+='onclick="parent.tmpSelectYearInnerHTML(this.innerText.substring(0,4))" title="点击这里选择年份"><span Author=j id=meizzYearHead></span></td>';
...
strFrame+='        title="点击这里选择月份"><span id=meizzMonthHead Author=j></span></td>';
strFrame+='        <td width=16 bgcolor=#ff9900 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='         onclick="parent.meizzNextM()" title="向后翻 1 月" Author=j><b Author=j>&gt;</b></td></tr>';
...
strFrame+='<tr Author="j" align=center valign=bottom><td style="font-size:12px;color:#FFFFFF" Author=j>日</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" Author=j>一</td><td style="font-size:12px;color:#FFFFFF" Author=j>二</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" Author=j>三</td><td style="font-size:12px;color:#FFFFFF" Author=j>四</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" Author=j>五</td><td style="font-size:12px;color:#FFFFFF" Author=j>六</td></tr>';
...
strFrame+='         Author=j title="' + _VersionInfo + '"><u>关闭</u></span>&nbsp;</td></tr>';
...
strFrame+='          <tr Author="j"><td Author=j align=left><input Author=j type=button class=button value="<<" title="向前翻 1 年" onclick="parent.meizzPrevY()" ';
strFrame+='             onfocus="this.blur()" style="font-size: 12px; height: 20px"><input Author=j class=button title="向前翻 1 月" type=button ';
strFrame+='             value="< " onclick="parent.meizzPrevM()" onfocus="this.blur()" style="font-size: 12px; height: 20px"></td><td ';
strFrame+='             Author=j align=center><input Author=j type=button class=button value=今天 onclick="parent.meizzToday()" ';
...
strFrame+='             Author=j type=button class=button value=">>" title="向后翻 1 年" onclick="parent.meizzNextY()"';
...
function tmpSelectYearInnerHTML(strYear) //年份的下拉框
{
  ...
  {
    if (i == m)
       {selectInnerHTML += "<option Author=j value='" + i + "' selected>" + i + "年" + "</option>\r\n";}
    else {selectInnerHTML += "<option Author=j value='" + i + "'>" + i + "年" + "</option>\r\n";}
  }
...
function tmpSelectMonthInnerHTML(strMonth) //月份的下拉框
{
  ...
  {
    if (i == m)
       {selectInnerHTML += "<option Author=j value='"+i+"' selected>"+i+"月"+"</option>\r\n";}
    else {selectInnerHTML += "<option Author=j value='"+i+"'>"+i+"月"+"</option>\r\n";}
  }
  selectInnerHTML += "</select>";
...
j_strFrame+='BORDER-BOTTOM: #ff9900 1px solid;BACKGROUND-COLOR: #fff8ec;font-family:宋体;}';
j_strFrame+='TD{FONT-SIZE: 9pt;font-family:宋体;}';
j_strFrame+='</style>';
j_strFrame+='<scr' + 'ipt>';
j_strFrame+='var j_datelayerx,j_datelayery;	/*存放日历控件的鼠标位置*/';
j_strFrame+='var j_bDrag;	/*标记是否开始拖动*/';
j_strFrame+='</scr' + 'ipt>';
...
j_strFrame+='        onclick="parent.j_meizzPrevM()" title="向前翻 1 月" Author=j><b Author=j>&lt;</b>';
...
j_strFrame+='onclick="parent.j_tmpSelectYearInnerHTML(this.innerText.substring(0,4))" title="点击这里选择年份"><span Author=j id=j_meizzYearHead></span></td>';
...
j_strFrame+='        title="点击这里选择月份"><span id=j_meizzMonthHead Author=j></span></td>';
j_strFrame+='        <td width=16 bgcolor=#ff9900 align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
j_strFrame+='         onclick="parent.j_meizzNextM()" title="向后翻 1 月" Author=j><b Author=j>&gt;</b></td></tr>';
...
j_strFrame+='          <tr Author="j"><td Author=j align=left><input Author=j type=button class=button value="<<" title="向前翻 1 年" onclick="parent.j_meizzPrevY()" ';
j_strFrame+='             onfocus="this.blur()" style="font-size: 12px; height: 20px"><input Author=j class=button title="向前翻 1 月" type=button ';
j_strFrame+='             value="< " onclick="parent.j_meizzPrevM()" onfocus="this.blur()" style="font-size: 12px; height: 20px"></td><td ';
j_strFrame+='             Author=j align=center><input Author=j type=button class=button value=完成 onclick="parent.jwntoday()" ';
j_strFrame+='             onfocus="this.blur()" title="选择完毕" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
j_strFrame+='             Author=j align=right><input Author=j type=button class=button value=" >" onclick="parent.j_meizzNextM()" ';
j_strFrame+='             onfocus="this.blur()" title="向后翻 1 月" class=button style="font-size: 12px; height: 20px"><input ';
j_strFrame+='             Author=j type=button class=button value=">>" title="向后翻 1 年" onclick="parent.j_meizzNextY()"';
...
window.frames.j_meizzDateLayer.document.close();		//解决ie进度条不结束的问题

//==================================================== WEB 页面显示部分 ======================================================
var j_outObject;
var j_outButton;		//点击的按钮
var j_outDate="";		//存放对象的日期
var j_MonHead = new Array(12);    		   //定义阳历中每个月的最大天数
...
var j_meizzTheYear=new Date().getFullYear(); //定义年的变量的初始值
var j_meizzTheMonth=new Date().getMonth()+1; //定义月的变量的初始值
var j_meizzWDay=new Array(39);               //定义写日期的数组

var j_odatelayer=window.frames.j_meizzDateLayer.document.all;		//存放日历对象
function j_setday2(tt,obj) //主调函数
{
	set_month_display=1;
	...
	j_outButton = (arguments.length == 1) ? null : th;	//设定外部点击的按钮
	//根据当前输入框的日期显示日历的年月
	...
	dads.display = '';
    //j_odatelayer.j_meizzYearHead.innerText  = j_meizzTheYear + " 年";
    //j_odatelayer.j_meizzMonthHead.innerText = j_meizzTheMonth + " 月";
	event.returnValue=false;
}
function j_setday(tt,obj) //主调函数
{
	set_month_display=0;
...
function j_meizzWriteHead(yy,mm)  //往 head 中写入当前的年与月
  {
	j_odatelayer.j_meizzYearHead.innerText  = yy + " 年";
	if(set_month_display==0)
    		j_odatelayer.j_meizzMonthHead.innerText = mm + " 月";
  }

function j_tmpSelectYearInnerHTML(strYear) //年份的下拉框
{
  if (strYear.match(/\D/)!=null){alert("年份输入参数不是数字！");return;}
  var m = (strYear) ? strYear : new Date().getFullYear();
  if (m < 1000 || m > 9999) {alert("年份值不在 1000 到 9999 之间！");return;}
...
function j_tmpSelectMonthInnerHTML(strMonth) //月份的下拉框
{
  if(set_month_display==0)
  {
  if (strMonth.match(/\D/)!=null){alert("月份输入参数不是数字！");return;}
...
function j_meizzSetDay(yy,mm)   //主要的写程序**********
{
  ...
  //alert(yy+"___"+mm);
 // for (var i = 0; i < 39; i++){j_meizzWDay[i]=""};  //将显示框的内容全部清空
...
function jwntoday()
{
    ...
    {
    	if(set_month_display==0)
			j_outObject.value= year + "-" + month ; //注：在这里你可以输出改成你想要的格式
		else
			j_outObject.value= year;
        j_closeLayer(); 
    }
    else {j_closeLayer(); alert("您所要输出的控件对象并不存在！");}
}

function j_meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  var yy=j_meizzTheYear;
  var mm = parseInt(j_meizzTheMonth)+ex;	//ex表示偏移量，用于选择上个月份和下个月份的日期
	//判断月份，并进行对应的处理
	if(mm<1){
		yy--;
		mm=12+mm;
  ...
  {
    if(set_month_display==0)
         j_outObject.value= yy + "-" + mm ; //注：在这里你可以输出改成你想要的格式
    else
