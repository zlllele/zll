	...
	$.rwdrag = {
		start : function(e) {
			...
			if (!$.rwdrag.current) {
				...
				// IE下需要把事件注册到打开的iframe上，否则无法触发
				$('iframe').each(function() {
					$this = $(this);
					if ($this.parent().css("display") == "block") {
						$this.contents().bind("mouseup", $.rwdrag.stop);
						$this.contents().bind("mousemove", $.rwdrag.drag);
					}
				});
		...
		drag : function(e) {
			...
			var left = (current.oleft + e.screenX - current.ox);
			var top = (current.otop + e.screenX - current.oy);
			if (top < 1)
				top = 0;
		...
		stop : function(e) {
			...
			$('iframe').each(function() {
				$this = $(this);
				if ($this.parent().css("display") == "block") {
					$this.contents().unbind("mouseup", $.rwdrag.stop);
					$this.contents().unbind("mousemove", $.rwdrag.drag);
				}
			});
